...
contract BaseToken is ERC20Capped, ERC20Burnable, ERC1363, Roles, TokenRecover {
    ...
    /**
     * @dev Status of minting request
     */
    enum MintStatus { Pending, Approved, Rejected, Executed }
     ...
     */
    modifier approved(uint reqID) {
        require(
            MintRequests[reqID].status == MintStatus.Approved,
            "BaseToken: Request to mint not approved by owner"
        );
        _;
    }
    ...
    function rejectMintRequest(uint reqID) public onlyOwner {
        MintRequests[reqID].status = MintStatus.Rejected;
    ...
    function createMintRequest(address to, uint256 amount) public onlyMinter {
        MintRequests.push(MintRequest(_msgSender(), to, amount, MintStatus.Pending));
